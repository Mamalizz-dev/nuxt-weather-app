...
export const useCallApi = () => {
    ...
    const getCurrentDataFromApi = (location: string) => {
        ...
        return new Promise((resolve: any, reject: any) => {
            try {           
                useLazyFetch<any>(`http://api.weatherapi.com/v1/current.json?key=${config.public.API_KEY}&q=${location}&aqi=yes`).then((response: any) => {
                    const { data } = response
    ...
    const getForecastDataFromApi = (location: string) => {
        ...
        return new Promise((resolve: any, reject: any) => {
            try {           
                useLazyFetch<any>(`http://api.weatherapi.com/v1/forecast.json?key=${config.public.API_KEY}&q=${location}&days=1&aqi=no&alerts=no`).then((response) => {
                    const { data } = response
    ...
    const getSearchDataFromApi = (query: string) => {
        ...
        return new Promise((resolve: any, reject: any) => {
            try {           
                useLazyFetch<any>(`http://api.weatherapi.com/v1/search.json?key=${config.public.API_KEY}&q=${query}`).then((response) => {
                    const { data } = response
